...
Emin_src = Parameter(
    1e2 * (1 + z) * u.GeV, "Emin_src", fixed=True
)
Emax_src = Parameter(
    1e8 * (1 + z)* u.GeV, "Emax_src", fixed=True
)
...
```python
sources = Sources()
ps = PointSource.make_powerlaw_source("txs", 5.7*u.deg, 77.3*u.deg, L, src_index, 0.3365, Emin_src, Emax_src,)
sources.add(ps)
```
...
```python
sources = Sources()
sources.add_diffuse_component(diffuse_norm, 1e5 * u.GeV, diff_index, Emin_diff, Emax_diff, )
...
```python
SimInfo.merge("bg.h5", "ps.h5", "combined.h5")
```

```python
bg = Events.from_file("bg.h5")
ps = Events.from_file("ps.h5")
comb = Events.from_file("combined.h5")
```
...
```python
assert np.all(np.isclose(np.array([ps.energies.to_value(u.GeV).tolist() + bg.energies.to_value(u.GeV).tolist()]) * u.GeV, comb.energies))
```

```python
assert np.all(np.isclose(np.vstack((ps.unit_vectors, bg.unit_vectors)), comb.unit_vectors))
```
