...
class ExposureIntegral():
    ...
    def __init__(self, source_list, effective_area, observation_time, minimum_energy, n_grid_points=50):
        """
        Handles calculation of the exposure integral, eps_k. 
        This is the convolution of the source spectrum and the 
        effective area, multiplied by the observation time. 
        ...
        self._alpha_grid = np.logspace(np.log(1.1), np.log(4.0), self._n_grid_points, base=np.e)
    ...
    def source_list(self, value):
        ...
        if not isintance(value, SourceList):
                ...
                if isinstance(source, DiffuseSource):
                    ...
                    for Em, EM in zip(self.effective_area._tE_bin_edges[:-1],
                        ...
                        for czm, czM in zip(self.effective_area._cosz_bin_edges[:-1],
                            ...
                            integral_grid_tmp[i] += E_integ * (ang_integ/(4*np.pi)) * aeff * np.power(1+z, 1-alpha) # GeV m^2
