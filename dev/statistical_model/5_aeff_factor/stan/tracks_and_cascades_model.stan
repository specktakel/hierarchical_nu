...
data {
  ...
  vector[Ngrid] E_grid_tracks;
  vector[Ngrid] Pdet_grid_tracks[Ns+1];
  vector[Ngrid] E_grid_cascades;
  vector[Ngrid] Pdet_grid_cascades[Ns+1];
...
transformed parameters {
  ...
  for (j in 1: N_cascades) {
	...
	lp_cascades[j, k] += log(1 / ( 4 * pi() ));
      ...
      /* Aeff factor */
      lp_cascades[j, k] += log(interpolate(E_grid_cascades, Pdet_grid_cascades[k], E_cascades[j]));
