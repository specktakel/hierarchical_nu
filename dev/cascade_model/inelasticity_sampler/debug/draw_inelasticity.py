...
class energy_slice(object):
    def __init__(self, data):
        ...
        # build inverse CDF
        x_vals = np.linspace(self.range_low, self.range_high, self.kde_nbins)
        self.x_vals = x_vals
        ...
        y_vals_cdf = np.array([cdf(tx) for tx in x_vals])
        ...
        self.inv_cdf = interp1d(y_vals_cdf[idx_first:], x_vals[idx_first:],  kind=5) 
    

    def eval_pdf(self, x):
        return self.kde.eval_point([x])/self.kde_norm
    

    def eval_cdf(self, x):
        val = quad(self.eval_pdf, self.range_low, x)[0]
...
class inelasticity_sampler(object):
    def __init__(self, infile, idx=0):
        self.idx = idx
        ...
        #self.samplers = [energy_slice(self.y_table[i]) for i in range(len(self.y_table)]
        #self.samplers = [energy_slice(self.y_table[i]) for i in range(1)]
        #self.samplers = [energy_slice(self.y_table[i]) for i in range(1)]
        self.samplers = [energy_slice(self.y_table[self.idx])]
    ...
    def sample_y(self, lE, nsamples=1):
        ...
        random_y = self.samplers[idx].sample_y(nsamples=nsamples)
        return random_y**2
