...
class HistogramSampler:
                ...
                for c, v in enumerate(n_reco):
                    ...
                    if v != 0.0:
                        ...
                        for c_psf, v_psf in enumerate(n_psf):
                            if v_psf != 0.0:
                                bins_ang = irf.ang_err_hists[etrue][dec][c][c_psf]
    ...
    def _make_ereco_hist_index(self, season: str) -> None:
        ...
        with get_ragged_index:
            ReturnStatement([f"dec + (etrue - 1) * {self._dec_bin_edges.size - 1}"])
    ...
    def _make_psf_hist_index(self, season: str) -> None:
        ...
        with get_ragged_index:
            n_dec_bins = self._dec_bin_edges.size - 1
            ReturnStatement(
                [f"ereco + (dec - 1) * 20 + (etrue - 1) * {n_dec_bins}  * 20"]
            )
    ...
    def _make_ang_hist_index(self, season: str) -> None:
        ...
        with get_ragged_index:
            ...
            ReturnStatement(
                [
                    f"psf + (ereco - 1) * 20 + (dec - 1) * 20 * 20 + (etrue - 1) * {n_dec_bins} * 20 * 20"
                ]
            )
    ...
    def generate_code(self) -> None:
        ...
        with self:
            if self.mode == DistributionMode.PDF:
                ...
                ereco_bins = StringExpression(["ereco_bins"])
                psf_hists = StringExpression(["psf_hists"])
                psf_bins = StringExpression(["psf_bins"])
                ...
                ereco_idx = ForwardVariableDef("ereco_idx", "int")
                ereco_idx << FunctionCall(
                    ["log_reco_energy", ereco_bins[etrue_idx, dec_idx]],
                    "binary_search",
                )
                ...
                psf_idx << FunctionCall(
                    [
                        psf_hists[etrue_idx, dec_idx, ereco_idx],
                        psf_bins[etrue_idx, dec_idx],
                    ],
                    "hist_cat_rng",
                )
                ...
                psf_ang << FunctionCall(
                    [
                        ...
                        FunctionCall(
                            [
                                psf_bins[etrue_idx, dec_idx, psf_idx],
                                psf_bins[etrue_idx, dec_idx, psf_idx + 1],
                            ],
                            "uniform_rng",
                        ),
                    ],
...
class R2021EnergyResolution(GridInterpolationEnergyResolution):
    ...
    def generate_code(self) -> None:
        ...
        if self.mode == DistributionMode.PDF:
            ...
            super().__init__(
                self._func_name,
                ["log_true_energy", "omega", "ereco_hists", "ereco_bins"],
        ...
        if self.mode == DistributionMode.RNG:
            ...
            with self:
                ereco_bins = StringExpression(["ereco_bins"])
                ereco_hists = StringExpression(["ereco_hists"])
                ...
                if self._make_ereco_cuts:
                    with WhileLoopContext([1]):
                        ereco << FunctionCall(
                            [
                                ereco_hists[etrue_idx, dec_idx],
                                ereco_bins[etrue_idx, dec_idx],
                            ],
                            "histogram_rng",
                        )
                        # Apply lower energy cut, Ereco_sim >= Ereco_data at the appropriate Aeff declination bin
    ...
    def prob_Edet_above_threshold(
        ...
        if use_interpolation:
            ...
            dec_idx = (
                np.digitize(np.sin(dec.to_value(u.rad)), self.irf.sin_dec_bin_edges) - 1
            )
            for cE, cD in product(
                range(self.irf.log_tE_bin_centers.size),
                range(self.irf.sin_dec_bin_centers.size),
                ...
                if upper_threshold_energy is None:
                    prob[(cE == idx_tE)] = 1.0 - self.irf.recoE_sampling[cE][cD].cdf(
    ...
    def generate_rng_function_code(self):
        ...
        UserDefinedFunction.__init__(
            ...
            [
                "real",
                "vector",
                "array[,,] real",  # ereco hists
                "array[,,] real",  # ereco bins
                "array[,,,] real",  # psf hists
                "array[,,] real",  # ang err bins
                "array[,,,,] real",  # ang err hists
                "array[,,] real",  # ang err bins
            ],
            "vector",
        )
        ...
        with self:
            ...
            log10Ereco << self.energy_resolution(
                log10Etrue, "omega", "ereco_hists", "ereco_bins"
            )
